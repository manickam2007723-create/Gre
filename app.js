...
window.loginAdmin = (e) => {
  ...
  if (email === 'manickam2007723@gmail.com' && pass === 'manickam@2007') {
      ...
      window.location.href = 'admin/index.html?fromLogin=true';
  } else {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.supabase && typeof SUPABASE_URL !== 'undefined' && !SUPABASE_URL.includes('YOUR_SUPABASE')) {
      ...
      console.warn("Supabase not configured locally. Using local storage products fallback.");
  }
  
  const localProducts = JSON.parse(localStorage.getItem('ecoMart_local_products')) || [];
  
  // Combine all products: newly added products (database or local) + default open source eco-friendly products
  products = [...fetchedProducts, ...localProducts, ...(window.initialProducts || [])];
