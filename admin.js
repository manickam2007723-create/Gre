...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (urlParams.get('fromLogin') === 'true' && sessionStorage.getItem('isAdminAuthenticated') === 'true') {
        // Came directly from successful main website login
        const overlay = document.getElementById('adminLoginOverlay');
        ...
        // Always require a password every time the page loads/re-loads otherwise
        sessionStorage.removeItem('isAdminAuthenticated');
        const overlay = document.getElementById('adminLoginOverlay');
...
document.getElementById('addProductForm').addEventListener('submit', async (e) => {
    ...
    const processProduct = async (finalImageStr) => {
        ...
        if (typeof SUPABASE_URL === 'undefined' || SUPABASE_URL.includes('YOUR_SUPABASE') || !supabase) {
            let localProducts = JSON.parse(localStorage.getItem('ecoMart_local_products')) || [];
            localProducts.push(newProduct);
            localStorage.setItem('ecoMart_local_products', JSON.stringify(localProducts));
            
            alert("Success! Product added locally to home page (Supabase not configured).");
...
async function loadProducts() {
    ...
    if (window.supabase && typeof SUPABASE_URL !== 'undefined' && !SUPABASE_URL.includes('YOUR_SUPABASE')) {
        ...
        // Fallback to local
        systemProducts = JSON.parse(localStorage.getItem('ecoMart_local_products')) || [];
    }
    ...
    // Sort newest first roughly based on ID
...
function loadOrders() {
    ...
    let allOrders = JSON.parse(localStorage.getItem('ecoMart_orders')) || [];
